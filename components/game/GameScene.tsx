  ...
  useEffect(() => {
    ...
    function applyPlayerSettings() {
      try {
        ...
        if (cfg.shirt) {
          shirtMat.color.set(cfg.shirt);
          shirtMat.emissive.set(cfg.shirt).multiplyScalar(0.12);
        }
    ...
    const auraGlow = new THREE.Mesh(
      new THREE.SphereGeometry(1.15, 10, 7),
      auraMat,
    );
    ...
    const auraRing1 = new THREE.Mesh(
      new THREE.TorusGeometry(1.0, 0.07, 6, 28),
      ringMat1,
    );
    ...
    const magnetRing = new THREE.Mesh(
      new THREE.TorusGeometry(2.0, 0.04, 6, 40),
      magnetRingMat,
    );
    ...
    function activateUlt() {
      if (ultKills < ULT_NEEDED || ultActive || !storeRef.current.running)
        return;
      ultKills = 0;
      ultActive = true;
      ultTimer = 180;
      ...
      for (let ei = enemies.length - 1; ei >= 0; ei--) {
        ...
        if (enemies[ei].hp <= 0) killEnemy(ei);
          ...
          enemies[ei].hitTimer = 20;
          spawnParticles(enemies[ei].mesh.position.clone(), 0xffdd00, 6);
        }
      }
      ...
      ultShockwave = new THREE.Mesh(
        new THREE.TorusGeometry(0.6, 0.35, 8, 40),
        swMat,
      );
    ...
    type RemoteEnemy = ReturnType<typeof buildEnemyRig> & {
      targetX: number;
      targetZ: number;
      animOffset: number;
    };
    ...
    const remotePickupEffects = new Map<
      number,
      { effect: string; color: number; name: string }
    >();
      ...
      syncIntervalId = setInterval(async () => {
        ...
        try {
          ...
          if (multiProps!.role === "host")
            body.hostBullets = bullets.map((b) => ({
              x: b.mesh.position.x,
              z: b.mesh.position.z,
              vx: b.vx,
              vz: b.vz,
            ...
            body.guestBullets = bullets.map((b) => ({
              x: b.mesh.position.x,
              z: b.mesh.position.z,
              vx: b.vx,
              vz: b.vz,
          ...
          if (res.ok) {
            ...
            if (other) {
              ...
              if (
                ...
                w.__pvpResult = "win";
                pendingRemoteHits = 0;
                fetch("/api/pvp/win", { method: "POST" }).catch(
                  () => undefined,
                );
              }
              if (
            ...
            if (
              multiProps!.mode === "coop" &&
              multiProps!.role === "guest" &&
              data.enemies
            ) {
            ...
            if (
              ...
              for (const ps of data.pickups) {
                ...
                if (!remotePickupMap.has(ps.id)) {
                  ...
                  remotePickupEffects.set(
                    ps.id,
                    puTypes.find((t) => t.effect === ps.effect) ?? {
                      name: ps.effect,
                      color: ps.color,
                      effect: ps.effect,
                    },
            ...
            if (
              multiProps!.mode === "coop" &&
              multiProps!.role === "host" &&
              data.pendingEnemyHits?.length
            ) {
            ...
            if (
              multiProps!.mode === "coop" &&
              data.coopGameOver &&
              !storeRef.current.gameOver
            )
              storeRef.current.damage(100);
    ...
    function animate() {
      ...
      // Day/night cycle
      {
        ...
        if (scene.fog)
          (scene.fog as THREE.Fog).color.copy(fogNight).lerp(fogDay, t);
        ...
        const sm = (scene.userData as { starMat?: THREE.PointsMaterial })
          .starMat;
      ...
      if (remoteRig) {
        remoteRig.root.position.x +=
          (remoteTargetX - remoteRig.root.position.x) * 0.2;
        remoteRig.root.position.z +=
          (remoteTargetZ - remoteRig.root.position.z) * 0.2;
        remoteRig.root.rotation.y +=
          (remoteTargetAngle - remoteRig.root.rotation.y) * 0.2;
      }
      ...
      if (pd > arenaRadius - 1) {
        playerMesh.position.x =
          (playerMesh.position.x / pd) * (arenaRadius - 1);
        playerMesh.position.z =
          (playerMesh.position.z / pd) * (arenaRadius - 1);
      }
      ...
      // Aura
      {
        ...
        auraMat.opacity +=
          ((hasEffect ? 0.22 + pulse * 0.18 : 0) - auraMat.opacity) * 0.1;
        auraMat.color.setHex(auraCol);
        ringMat1.opacity +=
          ((hasEffect ? 0.55 + pulse * 0.45 : 0) - ringMat1.opacity) * 0.1;
        ringMat1.color.setHex(auraCol);
        ringMat2.opacity +=
          ((hasEffect ? (0.55 + pulse * 0.45) * 0.65 : 0) - ringMat2.opacity) *
          0.1;
        ...
        magnetRingMat.opacity +=
          ((hasMagnet ? 0.25 + magnetPulse * 0.2 : 0) - magnetRingMat.opacity) *
          0.08;
      ...
      if (ultActive) {
        ...
        if (ultShockwave) {
          ...
          (ultShockwave.material as THREE.MeshBasicMaterial).opacity = Math.max(
            0,
            0.95 - prog * 2.2,
          );
      ...
      (window as unknown as Record<string, unknown>).__ult = {
        charge: ultKills,
        needed: ULT_NEEDED,
        active: ultActive,
      };
      ...
      if (!pvp.countdownActive && s.fireTimer >= s.fireRate) {
        ...
        if (multiProps?.mode === "pvp" && remoteRig) {
          ...
          remoteEnemyMap.forEach((re) => {
            ...
            if (dd < bestD) {
              bestD = dd;
              tx = re.group.position.x;
              tz = re.group.position.z;
            }
          });
        } else {
          const tgt = getNearestEnemy();
          if (tgt) {
            tx = tgt.mesh.position.x;
      ...
      // ── Upgrade: damage aura ─────────────────────────────────────────────────
      {
        ...
        const auraCount = upgrades.filter(
          (u: string) => u === "damage_aura",
        ).length;
      ...
      for (let ei = enemies.length - 1; ei >= 0; ei--) {
        ...
        if (multiProps?.mode === "coop" && remoteRig) {
          ...
          if (drx * drx + drz * drz < dlx * dlx + dlz * dlz) {
      ...
      if (frame % 3 === 0) {
        for (const sp of spectatorList) {
          ...
          sp.armL.rotation.z = 0.4 + cheer;
          sp.armR.rotation.z = -0.4 - cheer;
          sp.group.position.y =
            0.14 + Math.sin(frame * 0.18 + sp.animOffset) * 0.015;
        }
      }
      ...
      // Camera follow
      camera.position.x +=
        (playerMesh.position.x * 0.28 - camera.position.x) * 0.06;
      camera.position.z +=
        (playerMesh.position.z * 0.28 + 12 - camera.position.z) * 0.06;
    ...
    return () => {
      ...
      if (multiProps)
        fetch(`/api/rooms/${multiProps.roomId}/close`, {
          method: "POST",
        }).catch(() => undefined);
    };
  }, [mounted]);
