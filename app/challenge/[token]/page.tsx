...
import type { ChallengeObjectives } from "@/components/game/GameScene";
...
function ChallengeGameView({
  ...
  const [objectives, setObjectives] = useState<ChallengeObjectives>({
    ...
    async (score: number, wave: number, kills: number) => {
      ...
      try {
        const res = await fetch(
          `/api/challenges/${encodeURIComponent(token)}/submit`,
          {
            method: "POST",
    ...
    return (
      <div
        style={{
          minHeight: "100vh",
          background:
            "radial-gradient(ellipse at 50% 35%, rgba(46,204,113,0.15) 0%, #0a0010 60%)",
          color: "#fff",
          display: "flex",
      ...
      >
        ...
        <CheckCircle2
          size={64}
          color="#2ecc71"
          style={{ filter: "drop-shadow(0 0 20px #2ecc71)" }}
        />
        <Typography
          sx={{
            fontSize: 28,
            fontWeight: 900,
            color: "#2ecc71",
            textShadow: "0 0 28px rgba(46,204,113,0.5)",
          }}
        >
          DESAFIO CONCLUÍDO!
        </Typography>
        ...
        >
          <Typography
            sx={{ fontSize: 16, color: "rgba(255,255,255,0.6)", mb: 1 }}
          >
            Score
          </Typography>
          <Typography sx={{ fontSize: 36, fontWeight: 900, color: "#ffd700" }}>
    ...
    return (
      <div
        style={{
          minHeight: "100vh",
          background:
            "radial-gradient(ellipse at 50% 35%, rgba(50,10,20,0.97) 0%, #0a0010 60%)",
          color: "#fff",
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: 14, color: "rgba(255,255,255,0.5)" }}>
            Não completaste os objectivos do desafio.
          </Typography>
        </Box>
...
export default function ChallengePage() {
  ...
  if (expired) {
    return (
      ...
      >
        ...
        >
          Este desafio expirou após 24 horas.
        </Typography>
        <Button
  ...
  if (error || !challenge) {
    return (
      ...
      >
        <Typography sx={{ fontSize: 18, color: "rgba(255,100,100,0.7)" }}>
  ...
  if (playing) {
    return <ChallengeGameView challenge={challenge} token={token} username={(session?.user as { username?: string })?.username ?? ""} />;
  }
    ...
    >
      <Box sx={{ maxWidth: 500, mx: "auto" }}>
        ...
        <Box sx={{ textAlign: "center", mb: 3 }}>
          <Swords
            size={40}
            color={map?.accentColor ?? "#7b2ff7"}
            style={{
              filter: `drop-shadow(0 0 12px ${map?.accentColor ?? "#7b2ff7"}66)`,
            }}
          />
          <Typography
            sx={{
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
            ...
            <Typography
              sx={{ fontSize: 18, fontWeight: 800, color: "#aa55ff" }}
            >
              {challenge.creator}
            </Typography>
          </Box>
          ...
          challenge.targetKills ? (
            <Box sx={{ display: "flex", gap: 2, mb: 1.5, flexWrap: "wrap" }}>
              {challenge.targetScore && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  ...
                  <Typography
                    sx={{ fontSize: 14, fontWeight: 700, color: "#ffd700" }}
                  >
              ...
              {challenge.targetWaves && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  ...
                  <Typography
                    sx={{ fontSize: 14, fontWeight: 700, color: "#f39c12" }}
                  >
                    {challenge.targetWaves}
                  </Typography>
                  <Typography
              ...
              {challenge.targetKills && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  ...
                  <Typography
                    sx={{ fontSize: 14, fontWeight: 700, color: "#e74c3c" }}
                  >
                    {challenge.targetKills}
                  </Typography>
                  <Typography
            ...
            challenge.score > 0 && (
              ...
              >
                ...
                <Typography
                  sx={{ fontSize: 20, fontWeight: 900, color: "#ffd700" }}
                >
        ...
        {alreadyCompleted ? (
          ...
          >
            <Typography
              sx={{ fontSize: 14, color: "#e74c3c", fontWeight: 700, mb: 0.5 }}
            >
